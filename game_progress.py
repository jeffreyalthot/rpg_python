...
def outcome_for_tile(tile_kind: str, rng: Random) -> AdventureOutcome:
    if tile_kind == "battlefield":
        ...
        return AdventureOutcome("Escarmouche intense", xp, gold, hp_delta, item_found=rng.choice([None, "Rune ancienne"]))

    if tile_kind == "merchant":
        xp = rng.randint(7, 15)
        gold = rng.randint(4, 12)
        return AdventureOutcome("Transaction réussie avec un marchand", xp, gold, 0, item_found=rng.choice([None, "Talisman poli"]))

    if tile_kind in {"village", "starting_village"}:
        xp = rng.randint(8, 16)
        gold = rng.randint(5, 14)
        hp_delta = rng.randint(4, 12)
        return AdventureOutcome("Mission locale accomplie", xp, gold, hp_delta, item_found=rng.choice([None, "Ration de voyage"]))
